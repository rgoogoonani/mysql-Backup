...
# 🗄 MySQL Telegram / Bale Backup

بکاپ خودکار از دیتابیس‌های MySQL / MariaDB (یا فایل‌های SQLite)، فشرده‌سازی با zip، تقسیم خودکار به پارت‌های زیر ۵۰ مگابایت و ارسال به **تلگرام یا بله** — مخصوص سرورهای اوبونتو.
...
- ارسال به **تلگرام** یا **بله** (Bale) — موقع نصب **اول می‌پرسد** با کدام پیام‌رسان بفرستد. بله API کاملاً یکسان با تلگرام دارد و از داخل ایران بدون پروکسی در دسترس است
- پشتیبانی از **دو نوع دیتابیس**: **MySQL/MariaDB** (با `mysqldump`) و **SQLite / فایل‌محور**. موقع نصب، **اول از شما می‌پرسد** کدام را می‌خواهید
- برای SQLite کافی است **مسیر فایل(ها)** را بدهید — یک دیتابیس می‌تواند **چند فایل** باشد؛ همان فایل‌ها **دقیقاً همان‌طور که هستند** zip و ارسال می‌شوند (بدون پردازش)
...
- دسترسی `root` یا `sudo`
- MySQL 5.7+ یا MariaDB 10+ **یا** هر دیتابیس فایل‌محور (مثل SQLite)
- بسته‌های `mysql-client` ، `7zip` ، `zip` ، `curl` — اسکریپت نصب خودش نصب می‌کند
- یک ربات **تلگرام** یا **بله**
...
## 💬 انتخاب پیام‌رسان: تلگرام یا بله

**اولین سؤالِ نصب** این است که بکاپ‌ها با کدام پیام‌رسان فرستاده شوند:

```
--- Messenger ---
Where should the backups be sent?
  1) Telegram
  2) Bale (بله)
Choice [1]:
```

بله (Bale) یک پیام‌رسان ایرانی است که **API رباتش دقیقاً مثل تلگرام** است؛ فقط آدرس سرور فرق می‌کند: `https://tapi.bale.ai/bot<TOKEN>/METHOD`. مزیت مهمش این است که **از داخل ایران بدون پروکسی** در دسترس است، پس اگر بله را انتخاب کنید معمولاً به پروکسی نیازی ندارید.

</div>

<div dir="rtl">

## 🤖 مرحله ۱ — ساخت ربات و گرفتن چت آی‌دی
...
۱. در بله به **@BotFather** پیام بدهید (همان روند تلگرام)
۲. با `/newbot` ربات بسازید و توکن را بگیرید
۳. آدرس getUpdates در بله: `https://tapi.bale.ai/bot<TOKEN>/getUpdates`

**گرفتن چت آی‌دی:**

- **پیوی خودتان:** به ربات `/start` بدهید، بعد در مرورگر باز کنید:
  `https://api.telegram.org/bot<TOKEN>/getUpdates` (برای بله: `https://tapi.bale.ai/bot<TOKEN>/getUpdates`)
...
- در تلگرام روش ساده‌تر: به [@userinfobot](https://t.me/userinfobot) پیام بدهید.

> ⚠️ حتماً قبل از نصب، به ربات خودتان `/start` بدهید؛ وگرنه پیام‌رسان اجازهٔ ارسال پیام به شما را نمی‌دهد.
...
برای ساخت کاربر یک‌بار دسترسی ادمین لازم است. اسکریپت به‌ترتیب این راه‌ها را امتحان می‌کند و فقط اگر همه شکست بخورند سؤال می‌پرسد:

۱. اتصال سوکتی محلی با `--protocol=socket` (روش معمول روی اوبونتو، بدون پسورد)
۲. اتصال پیش‌فرض `mysql`
۳. کاربر نگهداری دبیان از `/etc/mysql/debian.cnf`
۴. در نهایت: پرسیدن یوزر و پسورد ادمین — که **جایی ذخیره نمی‌شود**

> اگر روی سرور دیتابیس ریموت باشد، کاربر با هاست `'%'` ساخته می‌شود.
...
## 🗃 حالت SQLite (دیتابیس فایل‌محور)

بعضی برنامه‌ها دیتابیس را به‌صورت یک فایل روی دیسک نگه می‌دارند (مثل SQLite). موقع نصب، **اولین سؤال** این است که چه چیزی می‌خواهید بکاپ بگیرید:
...
اگر گزینهٔ ۲ را بزنید، دیگر هاست/پورت/کاربر پرسیده نمی‌شود؛ فقط **مسیر فایل‌ها** را می‌خواهد. چون یک دیتابیس ممکن است **چند فایل** باشد، می‌توانید هر تعداد مسیر را (هر خط یک مسیر) وارد کنید و با زدن Enter روی خط خالی تمام کنید:

```
Enter the path to each file you want to back up, one per line.
A single database can be several files — add every file it uses.
The files are zipped exactly as they are. Press Enter on an empty line to finish.
  File path (empty to finish): /var/lib/myapp/data.db
  File path (empty to finish): /var/lib/myapp/cache.db
  File path (empty to finish):
```

نکته‌های مهم:

- **همان فایل‌هایی که می‌دهید، دقیقاً همان‌طور که هستند** برداشته و zip می‌شوند — نه dump گرفته می‌شود، نه پردازشی روی فایل انجام می‌شود و نه فایل اضافه‌ای برداشته می‌شود. (این حالت به `sqlite3` هم نیازی ندارد.)
- **همهٔ فایل‌ها با هم داخل یک آرشیو** قرار می‌گیرند (`sqlite_<تاریخ>.7z`)، تا موقع بازیابی همهٔ تکه‌های یک دیتابیس کنار هم باشند. بقیهٔ مسیر (فشرده‌سازی، تقسیم به پارت، رمز، پروکسی، ارسال دوره‌ای) دقیقاً مثل حالت MySQL است.
- چون فایل مستقیماً کپی می‌شود، بهتر است در لحظهٔ بکاپ برنامه در حال نوشتن سنگین روی فایل نباشد (برای SQLite معمولاً کپی یکجای فایل سالم است، ولی برای اطمینان کامل می‌توانید بکاپ را در ساعت کم‌ترافیک بگیرید).
- مسیرها با کاما در کانفیگ ذخیره می‌شوند، پس **مسیر فایل نباید کاما داشته باشد** (مسیرهای دارای فاصله مشکلی ندارند).

اجرای دستی حالت SQLite بدون فایل کانفیگ:

</div>

```bash
sudo mysql-telegram-backup.sh -t 123:ABC -c 987654321 \
  -e sqlite --sqlite-files "/var/lib/myapp/data.db,/var/lib/myapp/cache.db" -m 60
```
...
| نوع دیتابیس | `1` برای MySQL/MariaDB یا `2` برای SQLite |
| نام دیتابیس‌ها / مسیر فایل‌ها | حالت MySQL: مثلاً `wordpress,shop` یا `all` — حالت SQLite: مسیر فایل(ها) |
...
# ارسال با ربات بله به‌جای تلگرام
sudo mysql-telegram-backup.sh -g bale -t 123:ABC -c 987654321 -d "mydb" -p 'pass' -m 60

# بکاپ فایل‌های SQLite و ارسال با بله
sudo mysql-telegram-backup.sh -g bale -t 123:ABC -c 987654321 \
  -e sqlite --sqlite-files "/var/lib/myapp/data.db,/var/lib/myapp/cache.db" -m 60
```
...
| `-e` | `--engine` | نوع دیتابیس: `mysql` یا `sqlite` | `mysql` |
| `-d` | `--databases` | دیتابیس‌ها با کاما، یا `all` (حالت mysql) | — |
| | `--sqlite-files` | مسیر فایل‌های SQLite با کاما (حالت sqlite) | — |
...
آرشیو SQLite به شکل `sqlite_<تاریخ>.7z` می‌آید و داخلش یک پوشهٔ `sqlite_<تاریخ>/` هست که همهٔ فایل‌های دیتابیس داخل آن قرار دارند. کافی است آرشیو را باز کنید و هر فایل `.db` را سر جای خودش (مسیری که برنامه انتظار دارد) برگردانید. برنامه را قبل از جایگزینی متوقف کنید:

</div>

```bash
7z x sqlite_2026-09-12_10-00-00.7z           # لینوکس (یا Extract Here در ویندوز)
sudo systemctl stop myapp
cp sqlite_2026-09-12_10-00-00/data.db /var/lib/myapp/data.db
sudo systemctl start myapp
```

<div dir="rtl">

> فایل‌های `.db` فایل خام SQLite هستند و نیازی به `mysql` یا import ندارند — فقط جایگزین می‌شوند.
